...
import vn.backend.jobhunter.domain.response.resume.ResFetchResumeDTO;
import vn.backend.jobhunter.domain.response.resume.ResUpdateResumeDTO;
import vn.backend.jobhunter.domain.response.resume.ResFetchResumeDTO.UserResume;
import vn.backend.jobhunter.repository.JobRepository;
import vn.backend.jobhunter.repository.ResumeRepository;
...
public class ResumeService {
    ...
    public ResCreateResumeDTO handleCreateCV(Resume resume) {
        // check
    ...
    public boolean checkResumeExistByUserAndJob(Resume resume) {
        ...
        if (resume.getUser() != null) {
            Optional<User> userOp = this.userRepository.findById(resume.getUser().getId());
            if (userOp.isEmpty()) {
                return false;
            }
        ...
        if (resume.getJob() != null) {
            Optional<Job> jobOp = this.jobRepository.findById(resume.getJob().getId());
            if (jobOp.isEmpty()) {
                return false;
            }
    ...
    public ResUpdateResumeDTO handleUpdateCV(Resume resume) {
        // check
        resume = this.resumeRepository.save(resume);
    ...
    public void handleDeleteCV(long id) {
    ...
    public ResultPaginationDTO fetchAllResume(Specification<Resume> spec, Pageable pageable) {
        ...
        List<ResFetchResumeDTO> listResume = pageUser.getContent().stream().map(i -> this.getResume(i))
